...
const Notifications = props => {
    ...
    const {data, status} = useQuery(`notifications`, getNotifications(), {refetchInterval: 10000, refetchIntervalInBackground: true})
    let notifications;
    if (status == 'error') {
      ...
      notifications = data.notifications.map((notification) => (<NotificationBox notification={notification}/>))
    }
    return (
      <Box m={1}>
          {notifications}
